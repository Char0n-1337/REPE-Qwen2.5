...
5、对14 16 18 20层施加相同强度的测试结果，对上述四个层进行随机组合施加相同强度的控制向量，以及对上述层施加不同强度的测试结果（未完成）

6、对14 16 18 20四层进行单层强度注入实验，发现单层低强度-1.5注入均无法实现越狱，四层-1.5注入实现越狱，同时进行双层和三层的随机组合实验，但代码逻辑出现错误，没有清空之前的注入效果，因此需要重新实验修正结果。（未完成）

7、正向注入测试：结果发现正负向注入可以影响模型，但并非以0为对称点的镜像关系
